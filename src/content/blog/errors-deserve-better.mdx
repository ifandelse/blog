...
Yes, you caught me. 😀 The `isTimeout` is a custom helper that would check the shape of the error to test if it's a payment auth timeout error. The key difference, though, is that we've changed where this lives. We're building what Domain-Driven Design calls an [anti-corruption layer](https://ddd-practitioners.com/home/glossary/bounded-context/bounded-context-relationship/anticorruption-layer/). Our type-narrowing logic isn't clustered together in an endless sprawl of if/else branching. It's kept at the relevant edge - where we call the vendor SDK.
...
function reserveRoom(
  ...
  return Result.gen(async function* () {
    /*
      There's 3 integration patterns, depending on what the helper(s) return:
          - a bare `yield*` if it's sync & returns Result (like validateBookingRequest below)
          - `yield* Result.await(...)` if it return Promise<Result> (like the other 4 calls below)
          - OR, if you had a helper that was a plain Promise<T> that wasn't converted to resolve
            a Result, you'd wrap it with `Result.tryPromise` (like we did in the authorizePayment
            example above)
    */
...
This introduces the `Result.gen` behavior from better-result. `Result.gen` lets us treat a series of operations as if it were synchronous code (it's not), while allowing it to shortcut on the first error, _and_ giving our type system a union of the possible error types involved in the operation(s).
...
You can infer from the above snippet that `Result.match` handles success and failure scenarios (`ok` vs `err`). `matchError` gives us a clean way to discriminate between error types and choose how we react. (So, in essence, `match` answers "Did it work?" and `matchError` answers "Which way did it fail?") If we drop one of those error matches from `matchError`, the compiler will refuse to build.
...
I also strongly recommend exploring [Effect](https://www.effect.website/) as well. I [recently learned about them](https://x.com/_ashleypeacock/status/2047053156207497527) in a thread with [Ashley Peacock](https://x.com/_ashleypeacock). It's an astonishingly comprehensive TS ecosystem.
